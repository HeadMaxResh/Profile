...
public interface UserSoftSkillRepo extends JpaRepository<UserSoftSkill, Integer> {
    ...
    List<UserSoftSkill> findByRatedUserId(@Param("ratedUserId") Long ratedUserId);

    @Query("SELECT r FROM UserSoftSkill r WHERE r.raterUser.id = :raterUserId")
    List<UserSoftSkill> findByRaterId(@Param("raterUserId") Long raterUserId);
    ...
    Double findAverageRatingByUserAndSoftSkill(
            @Param("ratedUserId") Long ratedUserId,
